...
class DecisionTree(object):
    ...
    def predict(self, feature):
        ...
        # 单个样本分类
        def pre(tree, feat):
            if not isinstance(tree,dict):#isinstance() 函数用于检查一个对象是否是一个已知的类型
                return tree
            best_feature = list(tree.keys())[0]
            feat_value=feat[best_feature]
            if isinstance(tree[best_feature],dict):
                if feat_value in tree[best_feature].keys():
                    return pre(tree[best_feature][feat_value],feat)
                else:
                    return KeyError
            else:
                return tree[best_feature]
        return np.array([pre(self.tree,f) for f in feature])
